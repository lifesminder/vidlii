    ...
    if (isset($_GET["user"])) {
        ...
        if ($Exist) {
            ...
            if($Profile["channel_version"] == 3 || ($Profile["nouveau"] == 1 && $Profile["channel_version"] > 3)) {
                ...
                // Do a URL decode
                $Profile["channel_title"] = urldecode(urldecode($Profile["channel_title"]));
                $Profile["channel_description"] = urldecode(urldecode($Profile["channel_description"]));
                ...
                if(strlen($args["featured_channels"]) > 0) {
                    $featured_channels = [];
                    $args["featured_channels"] = explode(",", $args["featured_channels"]);
                    foreach($args["featured_channels"] as $channel) {
                        $channel = $api->db("SELECT displayname, channel_title, (select count(*) from subscriptions where subscription = '$channel') as subscribers from users where username = '$channel'");
                        if($channel["count"] == 1)
                ...
                if(strlen($args["featured_playlists"]) > 0) {
                    $featured_playlists = [];
                    $args["featured_playlists"] = explode(",", $args["featured_playlists"]);
                    foreach($args["featured_playlists"] as $playlist) {
                        $playlistAPI = new \Vidlii\Vidlii\API\Playlist($_SERVER["DOCUMENT_ROOT"]);
                        $playlist = $playlistAPI->index(["id" => $playlist]);
                        array_push($featured_playlists, $playlist);
                    }
                    $args["featured_playlists"] = $featured_playlists;
                }
                ...
                */
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST") { // prevent attacks possibilities
                    if(isset($_POST["save_channel_branding"]) && $_USER->logged_in && $_USER->username === $Profile["username"]) {
                        // Info
                        $channel_title = (isset($_POST["channel_title"]) && $_POST["channel_title"] != "") ? urlencode($_POST["channel_title"]) : "";
                        $channel_description = (isset($_POST["channel_description"]) && $_POST["channel_description"] != "") ? urlencode($_POST["channel_description"]) : "";
                        ...
                        // Background
                        $bg_color = (isset($_POST["bg_color"]) && !empty($_POST["bg_color"])) ? $_POST["bg_color"] : "#f0f0f0";
                        $bg_fixed = (isset($_POST["bg_fixed"]) && (int)$_POST["bg_fixed"] >= 0 && (int)$_POST["bg_fixed"] <= 1) ? (int)$_POST["bg_fixed"] : 0;
                        $bg_repeat = (isset($_POST["bg_repeat"]) && (int)$_POST["bg_repeat"] >= 1 && (int)$_POST["bg_repeat"] <= 4) ? (int)$_POST["bg_repeat"] : 0;
                        $bg_position = (isset($_POST["bg_position"]) && (int)$_POST["bg_position"] >= 1 && (int)$_POST["bg_position"] <= 3) ? (int)$_POST["bg_position"] : 0;
                        $bg_cover = (isset($_FILES["bg_cover"]) && $_FILES["bg_cover"]["error"] == 0) ? $_FILES["bg_cover"] : null;
                        $cover = null;
                        if($bg_cover != null) {
                            // To-Do: add checker of image
                            $cover = "\"".base64_encode(file_get_contents($bg_cover["tmp_name"]))."\"";
                        } else {
                            // If we need to remove the cover, simply delete it
                            if(isset($_POST["bg_remove"]) && (int)$_POST["bg_remove"] == 1)
                                $cover = "NULL";
                        }
                        ...
                        $update_query = "UPDATE users SET channel_title = '$channel_title', channel_description = '$channel_description', channel_tags = '$channel_tags', website = '$website', c_recent = $activity, c_comments = $comments, c_videos = $videos, c_favorites = $favorites, c_playlists = $playlists, a_last = $last_login, a_country = $country, a_age = $age, c_featured_channels = $featured_channels, c_featured_playlists = $featured_playlists, bg = \"$bg_color\", bg_fixed = $bg_fixed, bg_repeat = $bg_repeat, bg_position = $bg_position, featured_channels = \"$featured_title\"";
                        if($cover != null)
                            $update_query .= ", cover = $cover";
                        $update_query .= " WHERE displayname = '".$Profile["displayname"]."'";

                        $update_channel = $api->db($update_query);
                        if($update_channel["status"] >= 1) {
                            $link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
                            header("Location: ".$link);
                        } else {
                            notification($update_channel["message"], false, "error");
                        }
                        ...
                        if($comment != "") {
                            ...
                            notification("Your comment must have a content", false, "error");
                        }
                    } else if(isset($_POST["save_featured_channels"])) {
                        $featured_title = (isset($_POST["featured_title"]) && !empty($_POST["featured_title"])) ? $_POST["featured_title"] : "";
                        $featured_channels = (isset($_POST["featured_channels"]) && !empty($_POST["featured_channels"])) ? $_POST["featured_channels"] : "";
                        if(strlen($featured_channels) > 0) {
                            $featured_channels_list = explode(",", $featured_channels);
                            $featured_channels_list2 = [];
                            foreach($featured_channels_list as $channel) {
                                if(!empty($channel)) {
                                    $channel_exists = $api->db("SELECT displayname from users where displayname = \"$channel\"");
                                    if($channel_exists["count"] == 1) {
                                        array_push($featured_channels_list2, $channel_exists["data"]["displayname"]);
                                    }
                                }
                            }
                            $featured_channels = join(",", $featured_channels_list2);
                            $update_featured_channels = $api->db("UPDATE users set featured_title = \"$featured_title\", featured_channels = \"$featured_channels\" WHERE displayname = '".$Profile["displayname"]."'");
                            if($update_featured_channels["status"] < 0) {
                                notification($update_featured_channels["message"], false, "error");
                            }
                        }
                    } else if(isset($_POST["save_playlists"])) {
                        $featured_playlists = (isset($_POST["playlists"]) && !empty($_POST["playlists"])) ? $_POST["playlists"] : "";
                        if(strlen($featured_playlists) > 0) {
                            $featured_playlists_list = explode(",", $featured_playlists);
                            foreach($featured_playlists_list as $playlist) {
                                $channel_exists = (bool)$api->db("SELECT count(*) from playlists where purl = \"$playlist\"")["data"]["count(*)"];
                                if(!$channel_exists) {
                                    notification("Some of your playlists don't exist", false, "error");
                                }
                            }
                            $update_featured_playlists = $api->db("UPDATE users set playlists = \"$featured_playlists\" where displayname = '".$Profile["displayname"]."'");
                            if($update_featured_playlists["status"] < 0) {
                                notification($update_featured_playlists["message"], false, "error");
                            }
                        }
                    }
                } else if(isset($_GET["remove_comment"]) && (int)$_GET["remove_comment"] > 0) {
                    $remove_id = (int)$_GET["remove_comment"];
                ...
                if($Profile["channel_version"] == 3) {
                    switch($page) {
                        case "feed": {
                            if((bool)$args["owner"] == false && ($Profile["c_recent"] == 0 || $Profile["c_comments"] == 0)) {
                        ...
                        case "videos": {
                            ...
                            if(isset($_GET["query"]) && !empty($_GET["query"])) {
                                ...
                                $args["query"] = $query;
                                $args["videos"] = $api->db("SELECT url, title, description, uploaded_on, length, displayviews from videos where status > 1 and title like \"%$query%\" and uploaded_by = '".$Profile["displayname"]."' order by uploaded_on desc", true);
                                if($args["videos"]["count"] > 0) {
                                    $args["featured_video"] = $args["videos"]["data"][0];
                                    $args["featured_video"]["autoplay"] = 1;
                                    $args["featured_video"]["uploaded"] = get_time_ago($args["featured_video"]["uploaded_on"]);
