                ...
                */
                ...
                if(in_array($background_info["mime"], $allowed_types)) {
                    if(convert_filesize($background_info["size"], "mb") <= 5) {
                        ...
                        if($cover_update["status"] >= 1) notification("Background changed successfully", "/channel_theme", "green");
                        else notification($cover_update["message"], "/channel_theme", "red");
                    } else notification("Maximum background size is 5 MB", "/channel_theme", "red");
                } else notification("Invalid format! Consider uploading PNG/JPEG/WEBP/GIF", "/channel_theme", "red");
            } else {
                $cover_update = $api->db("UPDATE users set cover = null where username = \"".$api->session()["user"]["username"]."\"");
                if($cover_update["status"] >= 1) notification("Background removed successfully", "/channel_theme", "green");
                else notification($cover_update["message"], "/channel_theme", "red");
            }
