<?php
    ...
    function db($query) {
        $database = "mysqli://root@127.0.0.1/vidlii?charset=utf8mb4";
        $data = [];
    ...
    if(count($preConvs) >= 3) {
        foreach($preConvs as $x) {
            if($x == '.' || $x == '..') continue;
                ...
                $ffmpeg->Get_Info(); $ffmpeg->Make_Thumbnails(3, $x);

                if(file_exists("usfi/prvw/$id.file.jpg")) {
                    $updateThumb = db("update videos set thumbnail = '".base64_encode(file_get_contents("usfi/prvw/$id.file.jpg"))."' where url = '$id'");
                    if($updateThumb["status"] == 1) {
                        ...
                        if($makeConverting["status"] == 1) {
                            ...
                            if(file_exists("usfi/v/$id.mp4")) {
                                ...
                                if($deleteConverting["status"] == 1) {
                                    $deleteConvertingTotally = db("delete from converting where uri = '$id'");
                                    rename("usfi/v/$id.mp4", "usfi/v/$id..mp4");
