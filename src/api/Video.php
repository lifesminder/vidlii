	...
	class Video extends \Vidlii\Vidlii\API {
		function index(array $args, array $files = []) {
			...
			if(isset($args["id"]) && $args["id"] != "") {
				$id = $args["id"];
				$video_info = $this->db("SELECT url, title, description, tags, category, uploaded_on, uploaded_by, length, length as duration, displayviews as views, watched, comments, responses, favorites, featured from videos WHERE status > 1 and url = '$id'");
