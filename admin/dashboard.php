<?php
    ...
    if ($_USER->logged_in && ($_USER->Is_Admin || $_USER->Is_Mod)) {
        ...
        $Suggestions = $DB->execute("SELECT * FROM feature_suggestions INNER JOIN users ON feature_suggestions.from_user = users.username ORDER BY feature_suggestions.id DESC");
        $Bulletins = $DB->execute("SELECT * FROM bulletins INNER JOIN users ON users.username = bulletins.by_user ORDER BY bulletins.date DESC LIMIT 20");
